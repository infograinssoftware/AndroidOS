...
public class RecentlyViewFragment extends Fragment  implements rediuslick {

    private static final String TAG = RecentlyViewFragment.class.getSimpleName();
    Context context;
    View rootView;
    ...
    public static RecentlyViewFragment newInstance(String latitude, String logitude) {
        RecentlyViewFragment frag = new RecentlyViewFragment();
        latitudes = latitude;
        logitudes = logitude;
    ...
    public void callPropertySearchAPI(){
        ...
        RetrofitClient.getAPIService().getPropertySearchAPI(SharedPref.getSharedPreferences(context, Constants.TOKEN),
                latitudes,
                logitudes,
                "recently_view",this.radius).enqueue(new Callback<RetrofitUserData>() {
            @Override
